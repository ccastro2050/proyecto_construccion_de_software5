...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8046
EXPOSE 8046
...
| `EXPOSE 8046` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8046"]
    ...
    NAV -->|"localhost:8046"| APIFACTURAS
    ...
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13346"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15446"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11446"| SQLSERVER
```
...
```yaml
  postgres:
    ...
    ports:
      - "15446:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8046`, `localhost:15446`); entre contenedores, nombres de
...
docker network create proyecto_construccion_de_software5_default

# 2. postgres
docker run -d --name postgres --network proyecto_construccion_de_software5_default --restart unless-stopped `
  ...
  -p 15446:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver
docker run -d --name sqlserver --network proyecto_construccion_de_software5_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11446:1433 mcr.microsoft.com/mssql/server:2022-latest

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_construccion_de_software5_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init_sqlserver.sh

# 7. mariadb
docker run -d --name mariadb --network proyecto_construccion_de_software5_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_construccion_de_software5_default --restart unless-stopped `
...
```yaml
URL_API: http://api-facturas:8046
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8046` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8046` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8046:8046`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8046`, el front
...
```powershell
docker network inspect proyecto_construccion_de_software5_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion_de_software5` produce
la red `proyecto_construccion_de_software5_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion_de_software5-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion_de_software5_default has active endpoints (name:"proyecto_construccion_de_software5-api-facturas-1" …)
```
